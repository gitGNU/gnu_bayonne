...
bool voip::uri_dialed(char *buf, size_t size, const char *to, const char *id)
{
    buf[0] = 0;
    const char *schema = "sip:";

    if(!strncmp(to, "sips:", 5)) {
        schema = "sips:";
        to += 5;
    }
    else if(!strncmp(to, "sip:", 4)) {
        to += 4;
    }
    else if(!strncmp(id, "sips:", 5))
        schema = "sips:";
    ...
    if(host)
        ...
        if(!strncmp(id, "sips:", 5))
            id += 5;
        else if(!strncmp(id, "sip:", 4))
            id += 4;
        host = id;
    }
...
bool voip::uri_create(char *buf, size_t size, const char *server, const char *userid)
{
    buf[0] = 0;
    const char *schema="sip:";
...
bool voip::uri_server(char *buf, size_t size, const char *uri)
{
    buf[0] = 0;
    const char *schema="sip:";
...
bool voip::uri_userid(char *buf, size_t size, const char *uri)
{
	buf[0] = 0;
	char *ep;
...
unsigned short voip::uri_portid(const char *uri)
{
    const char *pp = NULL;
    const char *fp = NULL;
...
bool voip::uri_hostid(char *buf, size_t size, const char *uri)
{
    buf[0] = 0;
